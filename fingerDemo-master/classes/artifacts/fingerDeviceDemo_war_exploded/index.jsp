...
<!-- 设置天时段模态框 -->
	...
	<div class="container">
		...
		<button class="btn btn-primary" id="collect_emp_modal_btn">CollectPersonInfo</button>
	...
	<script type="text/javascript">
		...
		$("#setUserToDevice_emp_modal_btn").click(function(){
			...
			if(confirm("确定下发员工数据？")){
				//确认，发送ajax请求，删除
				$.ajax({
					url:"${APP_PATH}/setPersonToDevice",
					type:"GET",
					success:function(result){
						alert(result.msg);
						//回到当前页
    ...
    $("#logInfo_emp_modal_btn").click(function(){
    	...
    	window.location.href="http://localhost:8080/fingerDeviceDemo/logRecords.jsp";
	   ...
	   //保存天时段
		...
		$(document).on("click",".delete_btu",function(){
			...
			if(confirm("确认从机器上删除【"+empId +"】号员工吗？")){
				//确认，发送ajax请求，删除
				$.ajax({
					url:"${APP_PATH}/deletePersonFromDEvice?enrollId="+$(this).attr("delete-id")+"&backupnum=0",
					type:"GET",
					success:function(result){
						alert(result.msg);
						//回到当前页
		...
		$(document).on("click",".upload_btu",function(){
			...
			if(confirm("确认下发【"+empId +"】号员工吗？")){
				//确认，发送ajax请求，删除
				$.ajax({
					url:"${APP_PATH}/setOneUser?enrollId="+$(this).attr("upload-id"),
					type:"GET",
					success:function(result){
						alert(result.msg);
						//回到当前页
